...
class Data():
     ...
     def __init__(self):
         ...
         pass   
     def add_data(self,mae,mse,rmse,mape,coef,r2,pred,test,elapsed_time,r2_adj,z_name):
...
class MLSklean():
     ...
     def run(self, train_file, test_file,fold_name,fold):
            ...
            ##Call Algoritms
            self.processLM(fold_name,x_test.copy(),y_test.copy(),x_train.copy(),y_train.copy(),fold,z_name)
            self.processGBR(fold_name,x_test.copy(),y_test.copy(),x_train.copy(),y_train.copy(),fold,z_name)
            self.processXGB(fold_name, x_test.copy(),y_test.copy(),x_train.copy(), y_train.copy(),fold,z_name)
            self.processSVR(fold_name,x_test.copy(), y_test.copy(),x_train.copy(), y_train.copy(),fold,z_name)
            self.processRFR(fold_name,x_test.copy(), y_test.copy(),x_train.copy(), y_train.copy(),fold,z_name)
            self.processKNR(fold_name,x_test.copy(), y_test.copy(),x_train.copy(), y_train.copy(),fold,z_name)
            self.processLightGBM(fold_name,x_test.copy(), y_test.copy(),x_train.copy(), y_train.copy(),fold,z_name)
            self.processMPLRegressor(fold_name,x_test.copy(), y_test.copy(),x_train.copy(), y_train.copy(),fold,z_name)
            #comparation models
     ...
     # AdaBoostRegressor
     def processLM(self,fold_name,x_test, y_test,x_train, y_train,fold,z_name):
           ...
           self.add_value_vet(mae_lm,mse_lm,rmse_lm,mape_lm,coef_lm,r2_lm, r2_adj,lm_pred, y_test, elapsed_time,0,fold,z_name)


     ########################3Gradient Boosting Regression
     def processGBR(self,fold_name, x_test,y_test,x_train, y_train,fold,z_name):
          start_time = time.time()
          ...
          elapsed_time = time.time() - start_time
          self.add_value_vet(mae_clf,mse_clf,rmse_clf,mape_clf,coef_clf,r2_clf, r2_adj,clf_pred, y_test, elapsed_time,1,fold,z_name)

     ############XGBRegressor
     def processXGB(self,fold_name,x_test, y_test,x_train, y_train,fold,z_name):
         start_time = time.time()
         ...
         elapsed_time = time.time() - start_time
         self.add_value_vet(mae_xgb,mse_xgb,rmse_xgb,mape_xgb,coef_xgb,r2_xgb, r2_adj,xgb_pred, y_test, elapsed_time,2,fold,z_name)

     ################SVM Regression
     def processSVR(self,fold_name,x_test, y_test,x_train, y_train,fold,z_name):
         start_time = time.time()
         ...
         elapsed_time = time.time() - start_time
         self.add_value_vet(mae_svr,mse_svr,rmse_svr,mape_svr,coef_svr,r2_svr, r2_adj,svr_pred, y_test, elapsed_time,3,fold,z_name)

     ###############Random Forest Regressor
     def processRFR(self,fold_name,x_test, y_test,x_train, y_train,fold,z_name):
         start_time = time.time()
         ...
         elapsed_time = time.time() - start_time
         self.add_value_vet(mae_rfr,mse_rfr,rmse_rfr,mape_rfr,coef_rfr,r2_rfr, r2_adj,rfr_pred, y_test, elapsed_time,4,fold,z_name)

     # KNRegressor
     def processKNR(self,fold_name,x_test, y_test,x_train, y_train,fold,z_name):
         start_time = time.time()
         ...
         elapsed_time = time.time() - start_time 
         self.add_value_vet(mae_knr,mse_knr,rmse_knr,mape_knr,coef_knr,r2_knr, r2_adj,knr_pred, y_test, elapsed_time,5,fold,z_name)

     #Light GBM
     def processLightGBM(self,fold_name,x_test, y_test,x_train, y_train,fold,z_name):
          start_time = time.time()
          ...
          elapsed_time = time.time() - start_time
          self.add_value_vet(mae_lgb,mse_lgb,rmse_lgb,mape_lgb,coef_lgb,r2_lgb, r2_adj,lgb_pred, y_test, elapsed_time,6,fold,z_name)

     # MPLRegressor
     def processMPLRegressor(self,fold_name,x_test, y_test,x_train, y_train,fold,z_name):
         start_time = time.time()
         ...
         elapsed_time = time.time() - start_time
         self.add_value_vet(mae_perceptron,mse_perceptron,rmse_perceptron,mape_perceptron,coef_perceptron,r2_perceptron, r2_adj,perceptron_pred, y_test, elapsed_time,7,fold,z_name)

     def  add_value_vet(self,mae,mse,rmse,mape,coef,r2, r2_adj,pred, test, time, posi_model,fold,z_name):
        pred=self.sc_y.inverse_transform(pred, copy = True)
        test=self.sc_y.inverse_transform(test, copy = True)
        self.vet_fold[posi_model].add_data(mae,mse,rmse,mape,coef,r2,pred, test, time, r2_adj,z_name)
...
if __name__ == "__main__":
    ...
    for i in range(8):
            ...
            vec_z_name=[]
            for f, vec in enumerate(obj.vet_z_name):
                print(vec)
                for ff,vec2 in enumerate(vec):
                    vec_z_name.append(vec2)
            vet_best_frame=[]
            posi=0
            name_best=''
            for d in range(512): #numero total de imagens
                print(vec_pred[d])
                if(posi==0):
                  minor=abs(vec_pred[d]-vec_test[d])
                  name_best=vec_z_name[d]
                  posi+1
                else:
                  if(minor>abs(vec_pred[d]-vec_test[d])):
                     minor=abs(vec_pred[d]-vec_test[d])
                     name_best=vec_z_name[d]
                  posi+1
                if(posi==15):#quantidade de frames por pasta -1
                  vet_best_frame.append(name_best)
                  posi=0                         
            print(obj.vet_test)
            ml.write_log('Pred:('+str(len(vec_pred))+') '+ml.vet_text(vec_pred))
            ml.write_log('Test:('+str(len(vec_test))+') '+ml.vet_text(vec_test))
            ml.write_log('Best Images:('+str(len(vet_best_frame))+') '+ml.vet_text(vet_best_frame))
