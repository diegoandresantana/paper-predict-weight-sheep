...
class Snake:
    ...
    # Constants
    MIN_DISTANCE_BETWEEN_POINTS = 3    # The minimum distance between two points to consider them overlaped
    ...
    beta = 0.6          # The weight of the curvature energy.
    delta = 0.6         # The weight of the user configured energy.
    w_line = 0.5        # The weight to the line energy.
    w_edge = 0.8        # The weight to the edge energy.
    w_term = 0.5        # The weight to the term energy.
    iteracao = 0
    #nome_image = None         #nome da imagen

    PONTOS = []
    #arq = open('DadosExtraidosSnake.txt', 'w') #r ou a
    #arqNovo = open('DadosArtigoSnake.txt', 'a')  # r ou a
    # texto = " Medidas: \n"
    #arq.write('MEDIDAS:\n')
    ...
    def __init__( self, image = None,  closed = True ):
        ...
        def rotacionar(image, cnt):
            if len(cnt) == 0:
                result = cv2.findContours(image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
                contours, hierarchy = result if len(result) == 2 else result[1:3]
                cnt = contours[0]

            ellipse = cv2.fitEllipse(cnt)
            # cv2.ellipse(image, ellipse, (0, 150, 255), 2)
            (x, y), (MA, ma), angle = ellipse

            # rotacionar imagem
            # for angle in np.arange(0, 360, 15):
            image = imutils.rotate_bound(image, 90 - angle)
            # cv2.imshow("Rotated (Correct)", rotated)
            return image, angle


        def padding(img_size, bg_size):
            from numpy import ndarray, zeros
            from matplotlib import pyplot as plt

            img = img_size
            # img = ndarray(img_size, uint8)
            # img[:, :, :] = 255  # imagem toda branca
            # bg = zeros(bg_size, uint8)  # imagem maior toda preta
            bg = bg_size
            w, h = img.shape[:2]
            bw, bh = bg.shape[:2]
            nw, nh = (bw - w) // 2, (
                        bh - h) // 2  # calculo das coordenasas iniciais onde a imagem ira ficar na imagem maior
            newimg = bg.copy()
            newimg[nw: nw + w, nh: nh + h] = img
        ...
        img_gray, angle = rotacionar(img_gray, cnt)

        image = imutils.rotate_bound(img, 90 - angle)

        img2 = cv2.imread('fundo.png')
        image = padding(image, img2)  # (img_size=(64, 64, 3), bg_size=(66, 66, 3))
    ...
    def linha( self ):
        ...
        atual = 0
        img = self.image.copy()
        #nome = self.nome_image

        lista_x = []
        lista_y = []
        lista_ordenada = []
        # for i in range(1, self.points[:, 0].size):
            # lista_x.append(float(self.points[i][1]))

        minimo_ponto = np.min(self.points, axis=0)
        x_minimo = minimo_ponto[0]
        y_minimo = minimo_ponto[1]

        maximo_ponto = np.max(self.points, axis=0)
        x_maximo = maximo_ponto[0]
        y_maximo = maximo_ponto[1]

        lista = 1
        lista_ordenadissima = []
        #ordenar apos encontrar o ponto mais distante.
        labels = []
        values = []
        types= []
        while P < len(self.points):
            liga = len(self.points)
            ponto_atual = self.points[atual]
            if (ponto_atual[1] == y_maximo):
                lista = 2

            if (lista == 1):
                lista_x.append(self.points[atual])

            if (lista == 2):
                lista_y.append(self.points[atual])

            if (P == 19):# troquei aqui 49
                lista_ordenada = lista_y + lista_x
                
                # print(lista_ordenada)
                atual = 0
                # calcular distancia apos nova ordenacao.
                #cabecalho = '@dadosboi \r\n'   #cabecalho do arff
                #texto = ''
                
                while L < len(lista_ordenada):
                    liga = len(lista_ordenada)

                    while atual < liga:

                        distancia = dist.euclidean(lista_ordenada[atual], lista_ordenada[liga - 1])
                        ## cm = distancia/(0.955/12.192)
                        liga -= 1
                        #cabecalho =  cabecalho + "  @attribute '" + str(atual) +"-"+ str(liga) +"' real \r\n"
                        labels.append(str(atual) +"-"+ str(liga))
                        values.append(distancia)
                        #print("Distancia %s - %s-%s = %f" % (nome, atual, liga, distancia))
                        #self.arq.write("%s; %s-%s ; %f" % (nome, atual, liga, distancia) + "\n")
                        #if (atual < 49):
                            #texto = texto + str(distancia) + ","
                            #self.arqNovo.write("%s," % (texto))
                            #print('Texto: ',texto , '\n')
                        if (atual == 49):
                            #texto = texto + str(distancia)+ "," + str(self.peso) + "," + str(self.nome_image)
                            #self.arqNovo.write("%s" % (texto) +' \n')
                            #print('Texto: ',texto)
                            lista_ordenadissima = lista_ordenada
                            types = ['numeric'] * len(labels)    

                    atual += 1
                    L += 1
                    
                #self.arqNovo.write("%s" % (cabecalho) + " \n")
                #print('passei aqui')
                #self.arqNovo.write("%s" % (texto) + " \n")
            atual += 1
            P += 1

        return lista_ordenadissima, labels, types, values
    ...
    def visualize( self ):
        ...
        result = cv2.findContours(self.gray, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        contours, hierarchy = result if len(result) == 2 else result[1:3]
        (x1, y1, w1, h1) = cv2.boundingRect(contours[0])
        #[cv2.rectangle(img, tuple(x), (x1 + w1, y1+ h1), (0, 255, 255), 1) for x in self.points]
        #[ cv2.rectangle(img, tuple(x), (x1 + w1, y1+ h1), point_color, -1) for x in self.points]
        #[cv2.ellipse(	img, tuple(x), thickness, (0, 255, 0), -1) for x in self.points]
        ...
        for x in self.points:
            cv2.putText(img, 'Iteracao:'+str(self.iteracao), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.circle(img, tuple(x), thickness, point_color, -1)
            ...
            contador = contador + 1
            #imprime os numeros
            #cv2.putText(img, str(contador), tuple(x), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
            # cv2.putText(image, 'OpenCV', org, font, fontScale, color, thickness, cv2.LINE_AA)
            if contador == 50:
                contador = 0
            """
            if self.iteracao == 10:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 2000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 4000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 6000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 8000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 10000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 12000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 14000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 16000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            if self.iteracao == 18000:
                cv2.imshow('image_'+str(self.iteracao), img)
                cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
            """
            # if cv2.waitKey(20) & 0xFF == ord('q'):
            if self.iteracao >=20000:#19999:

                #cv2.imshow('image_'+str(self.iteracao), img)
                #cv2.imwrite('/segmento_'+str(self.iteracao)+'.jpg', img)
                self.PONTOS.append(self.points)
                              
                print('Sistema encerrado apos:', str(self.iteracao))
                cv2.imwrite('teste_linha.jpg', img)
                lista,labels,types,values = self.linha()
                #exit()
                break

        return img,labels,types,values
